...
const core = fs.readFileSync(path.join(root, "scripts/lib/oisummit/capture-core.mjs"), "utf8");
...
test("API contract is protected and dedicated", () => {
  ...
  assert.match(core, /OISUMMIT_CAPTURE_WRITE_ENABLED/);
  assert.match(core, /OISUMMIT_CAPTURES_TABLE_NAME/);
  assert.match(api, /writeCaptureToAirtable/);
  assert.match(core, /capture_id/);
  assert.match(core, /duplicate/);
  assert.match(core, /CONFIG_REQUIRED/);
  assert.match(api, /timingSafeEqual/);
  assert.doesNotMatch(core, /AIRTABLE_TABLE_NAME\s*\|\|/);
});
