(function () {
  ...
  if (window.__ariFvAnimationInit) return;

  var MASTER = window.ARI_FV_MASTER;
  if (!MASTER || typeof window.gsap === 'undefined') return;

  var gsap = window.gsap;
  var root = document.getElementById('ari-fv-animation');
  if (!root) return;

  window.__ariFvAnimationInit = true;
  ...
  function renderSearchAi() {
    return (
      '<p class="ari-fv-ai-text">条件に合う候補を探しています</p>' +
      '<p class="ari-fv-ai-sub">検索中…</p>' +
  ...
  function boot() {
    if (window.gsap && window.ARI_FV_MASTER) init();
